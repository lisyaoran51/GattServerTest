...
cmdNoleadv(int hdev)
{
  ...
  if (status)
  {
	  printf("Disabling LE advertise on hci%d returned error status:%d.\n",
      hdev, status);
  }
}
...
cmdLeadv(int hdev)
{
  ...
  if (status)
  {
	  printf("Enabling LE advertise on hci%d returned error status:%d\n",
        hdev, status);
  }
}
...
updateDeviceName(string const& name)
{
  ...
  printf("device set new name = %s\n", getDeviceNameFromFile().c_str());
}
...
hcitoolCmd(int dev_id, vector <string> const& args)
{
  ...
  {
	  printf("ogf must be in range (0x3f,0x3ff)\n");
  }
...
{
	printf("starting beacon %s on hci%d\n", name.c_str(), deviceId);
